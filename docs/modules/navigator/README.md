...
This module page describes Navigator's Product API and local-host
implementation: the pinned Harness adapters, Rust native host, and Python
session persistence and Product read service.

本模块页说明 Navigator 的 Product API 与本地主机实现：固定版本 Harness 适配器、Rust
原生宿主，以及 Python 会话持久化与 Product 读取服务。
...
| [`../../desktop/README.md`](../../desktop/README.md) | UI extraction and ownership record / UI 迁移与所有权记录 |
...
3. `docs/desktop/README.md` — read the UI extraction and ownership record.
...
3. `docs/desktop/README.md` —— 阅读 UI 迁移与所有权记录。
